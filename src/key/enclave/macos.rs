...
const PRIVATE_KEY_NAME: &str = "com.sekey.priv";
const PUBLIC_KEY_NAME: &str = "com.sekey.pub";

static ERR_SEC_DUPLICATE_ITEM: OSStatus = -25299;
static ERR_SEC_SUCCESS: OSStatus = 0;

type SecAccessControlCreateFlags = u32;
static K_SEC_ACCESS_CONTROL_USER_PRESENCE: u32 = 1 << 0;
...
impl SKey {
    unsafe fn sec_item_copy_matching(dict: CFDictionary) -> Vec<CFDictionary> {
        let mut items = vec![];
    ...
    pub fn get_public_keys() -> Vec<PubKey> {
        ...
        unsafe {
            ...
            let keys = SKey::sec_item_copy_matching(dict);
            // iter thru the keys and the get information from the key dict.
            for key in keys {
                pub_keys.push(SKey::get_pubkey_from_cfdictionary(key));
            }
        }
        pub_keys
    }
    ...
    unsafe fn get_public_ref(hash: Vec<u8>) -> Result<CFTypeRef, &'static str> {
        ...
        let dict = CFDict::new()
            ...
            .add_boolean(kSecReturnAttributes, true)
            .get();
        let mut keys = SKey::sec_item_copy_matching(dict);
    ...
    pub fn get_public_key(hash: Vec<u8>) -> Result<PubKey, &'static str> {
        let key: PubKey;
        unsafe {
            let keyref = SKey::get_public_ref(hash)?;
            let keyref = CFDictionary::wrap_under_get_rule(keyref as CFDictionaryRef);
            key = SKey::get_pubkey_from_cfdictionary(keyref);
        }
    ...
    pub fn sign_data(data: Vec<u8>, key_hash: Vec<u8>) -> Result<Vec<u8>, &'static str> {
        ...
        unsafe {
            ...
            let keyref = SKey::get_private_ref(key_hash)?;
    ...
    pub fn delete_keypair(hash: Vec<u8>) -> Result<(), &'static str> {
        unsafe {
            SKey::delete_private_key(hash.clone());
            SKey::delete_public_key(hash);
        }
        Ok(())
    }
    ...
    pub fn generate_keypair(label: String) -> Result<(), &'static str> {
        unsafe {
            ...
            let access_control = SecAccessControlCreateWithFlags(
                ...
                K_SEC_ACCESS_CONTROL_USER_PRESENCE | K_SEC_ACCESS_CONTROL_PRIVATE_KEY_USAGE,
                &mut error,
            );
            ...
            if status != ERR_SEC_SUCCESS {
                let msg = SecCopyErrorMessageString(status, std::ptr::null());
                let msg = CFString::wrap_under_get_rule(msg).to_string();
                dbg!(msg);
                return Err("Error creating keypair");
            }
            SKey::save_public_key(public_key_ref, label);
        }
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn create() {
        SKey::generate_keypair("key".to_owned()).expect("Key created");
        for kp in SKey::get_public_keys() {
            let PubKey { label, hash, .. } = kp;

            if label == "key" {
                SKey::delete_keypair(hash).expect("Delete the keypair 'key'")
            }
        }
    }
}
